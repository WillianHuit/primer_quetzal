...
var TEXTOS_EN = {
  ...
  datos: {
    ...
    // Las siete tareas de rama: una por cada rama del diversificado
    logica: { nombre: 'Follow the pattern',
      descripcion: 'Four numbers with a hidden rule. Which one comes next?',
      ensena: 'Seeing the pattern before you touch anything is half of programming.' },
    primeros: { nombre: 'What comes first?',
      descripcion: 'Someone got hurt. Of three sensible things, which one goes first?',
      ensena: 'In an emergency what you decide is not what to do, but what to do first.' },
    margen: { nombre: 'Which one leaves more?',
      descripcion: 'Two products with their cost and price. Which one leaves you more?',
      ensena: 'What a business earns is not what it sells: it is what is left over.' },
    centrado: { nombre: 'Which one is centred?',
      descripcion: 'Four frames. In only one is the shape right in the middle.',
      ensena: 'The eye that spots two millimetres off is the whole craft of design.' },
    encaja: { nombre: 'Which piece fits?',
      descripcion: 'One gap and three pieces. Only one fits. By eye.',
      ensena: 'Sizing something up by eye can be learned, and in a workshop it saves the day.' },
    siembra: { nombre: 'When do you plant?',
      descripcion: 'The countryside runs on the calendar. When is each thing due?',
      ensena: 'Farming and tourism do not sell a product: they sell a season.' },
    explicar: { nombre: 'Explain it to a child',
      descripcion: 'Three true explanations. Only one lands the first time.',
      ensena: 'Teaching is not knowing: it is finding the sentence that makes it click.' },
